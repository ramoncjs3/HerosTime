	...
	for batchIdx := 0; batchIdx < nBatches; batchIdx++ {
		for kernelIdx := 0; kernelIdx < nKernels; kernelIdx++ {
			...
			for h := 0; h < paddedX.Shape()[2]; h += strideSize {
				...
				// gorgonia drops dimensions that are sliced down to size 1,
				// so align the kernel view with the sub image shape explicitly.
				if err := subKernel.Reshape(subImage.Shape()...); err != nil {
					return nil, err
				}

				subImageShape := subImage.Shape()
				subKernelShape := subKernel.Shape()
				subImage, subKernel, err = ops.UnidirectionalBroadcast(subImage, subKernel)
				if err != nil {
					return nil, fmt.Errorf("conv1d broadcast subImage=%v subKernel=%v x=%v kernel=%v: %w", subImageShape, subKernelShape, x.Shape(), kernel.Shape(), err)
				}
		...
		for kernelIdx := 0; kernelIdx < nKernels; kernelIdx++ {
			...
			for h := 0; h < paddedX.Shape()[2]; h += c.strides[0] {
				...
				for w := 0; w < paddedX.Shape()[3]; w += c.strides[1] {
					...
					subImage, subKernel, err = ops.UnidirectionalBroadcast(subImage, subKernel)
					if err != nil {
						return nil, fmt.Errorf("conv2d broadcast subImage=%v subKernel=%v x=%v kernel=%v h=%d w=%d: %w", subImageShape, subKernelShape, paddedX.Shape(), kernel.Shape(), h, w, err)
					}
...
func (c *Conv) addBias(out, bias tensor.Tensor) (tensor.Tensor, error) {
	...
	outShapeBefore := out.Shape()
	biasShapeBefore := bias.Shape()
	out, bias, err = ops.UnidirectionalBroadcast(out, bias)
	if err != nil {
		return nil, fmt.Errorf("conv addBias broadcast out=%v bias=%v: %w", outShapeBefore, biasShapeBefore, err)
	}
