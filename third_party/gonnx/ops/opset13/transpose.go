...
func (t *Transpose) Apply(inputs []tensor.Tensor) ([]tensor.Tensor, error) {
	...
	if _, ok := inputs[0].(*tensor.Dense); !ok {
		return nil, fmt.Errorf("transpose input type %T shape %v perm %v is not *tensor.Dense", inputs[0], inputs[0].Shape(), t.perm)
	}
	out, err := tensor.Transpose(inputs[0], t.perm...)
	if err != nil {
		return nil, fmt.Errorf("transpose shape %v perm %v: %w", inputs[0].Shape(), t.perm, err)
	}
