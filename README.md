...
暴走（4399）渠道短时间多次登录会触发图形验证码。程序内置了 OCR 自动识别：登录页出现验证码输入框时，自动拉取验证码图片、识别后连同账号密码重新提交，识别失败会自动刷新重试，最多 `captcha.max_attempts` 次；首次命中 4399 限流（“请稍后再试”）时立即结束本轮，并为账号记录 `captcha.rate_limit_cooldown` 冷却，后续刷新在冷却结束前不会再次登录。非验证码导致的登录失败（如密码错误）不会重试。
